...
//    public class DemoReportStorageWebExtension : ReportStorageWebExtension, IReportProviderAsync {
...
//            httpContextAccessor.HttpContext?.Request.Cookies.TryGetValue(ReportingCookieHelper.IdCookieKey, out xrId);
//            if(ShouldSetCookies && string.IsNullOrEmpty(xrId)) {
//                xrId = await ReportingCookieHelper.SetSessionID(jsRuntime);
//            }
//            return xrId;
//        }
//        public async Task<string> GetReportFolderPathAsync() {
//            var sessionID = await GetGuidAsync();
//            var folderPath = Path.Join(TempReportsFolderPath, sessionID);
//            if(!Directory.Exists(folderPath) && !String.IsNullOrEmpty(sessionID)) {
