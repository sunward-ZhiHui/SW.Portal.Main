...
namespace Application.Handlers.CommandHandler.LevelMasters
{
    public class CreateLevelMasterHandler : IRequestHandler<CreateLevelMasterCommand, LevelMasterResponse>
    {
        ...
        public async Task<LevelMasterResponse> Handle(CreateLevelMasterCommand request, CancellationToken cancellationToken)
        {
            ...
            uid = Guid.NewGuid();
            await _HRMasterAuditTrailQueryRepository.InsertHRMasterAuditTrail("LevelMaster", "Add", null, queryEntity?.ModifiedByUserId?.ToString(), data, guid, queryEntity?.AddedByUserId, DateTime.Now, false, "ModifiedByUserID", uid);
            uid = Guid.NewGuid();
            await _HRMasterAuditTrailQueryRepository.InsertHRMasterAuditTrail("LevelMaster", "Add", null, queryEntity?.ModifiedDate != null ? queryEntity.ModifiedDate.Value.ToString("dd-MMM-yyyy hh:mm:ss tt") : null, data, guid, queryEntity?.AddedByUserId, DateTime.Now, false, "ModifiedDate", uid);
            uid = Guid.NewGuid();
            await _HRMasterAuditTrailQueryRepository.InsertHRMasterAuditTrail("LevelMaster", "Add", null, queryEntity?.ModifiedBy, data, guid, queryEntity?.AddedByUserId, DateTime.Now, false, "ModifiedBy", uid);
