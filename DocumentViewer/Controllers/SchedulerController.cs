...
namespace DocumentViewer.Controllers
{
    public class SchedulerController : Controller
    {
        ...
        {
            var query = from oal in _context.PlanningForProductionProcessByMachine
                        join oau in _context.ApplicationMasterDetail on oal.ProductionPlanningProcessId equals oau.ApplicationMasterDetailId
                        select new PlanningForProductionProcessByMachine
                        {
                            PlanningForProductionProcessByMachineId = oal.PlanningForProductionProcessByMachineId,
                            TypeOfProductionId = oal.TypeOfProductionId,
                            SquenceForPlanning = oal.SquenceForPlanning,
                            ProductionPlanningProcessId = oal.ProductionPlanningProcessId,
                            TypeOfProcessId = oal.TypeOfProcessId,
                            ProductionPlanningProcess = oau.Value
                        };
            var res = query.ToList();
            var Linequery = from oal in _context.PlanningForProductionProcessByMachineRelated
                            join oau in _context.ApplicationMasterDetail on oal.FixAssetMachineNameRequipmentId equals oau.ApplicationMasterDetailId
                            select new PlanningForProductionProcessByMachineRelated
                            {
                                PlanningForProductionProcessByMachineId = oal.PlanningForProductionProcessByMachineId,
                                PlanningForProductionProcessByMachineRelatedId = oal.PlanningForProductionProcessByMachineRelatedId,
                                FixAssetMachineNameRequipmentId = oal.FixAssetMachineNameRequipmentId,
                                AssetIDWithModelId = oal.AssetIDWithModelId,
                                PlanningForProductionProcessByMachine = oau.Value
                            };

            var Lineres = Linequery.ToList();
            List<SchedulerDisplay> Group1 = new List<SchedulerDisplay>();
            List<SchedulerDisplay> Group2 = new List<SchedulerDisplay>();
            if (res?.Any() == true)
            {
                res.ForEach(s =>
                {
                    var exits = Group1.Where(w => w.Id == s.ProductionPlanningProcessId).FirstOrDefault();
                    if (exits == null)
                    {
                        SchedulerDisplay schedulerDisplay = new SchedulerDisplay();
                        schedulerDisplay.Id = s.ProductionPlanningProcessId;
                        schedulerDisplay.Text = s.ProductionPlanningProcess;
                        schedulerDisplay.Color = GetRandomColor();
                        schedulerDisplay.PlanningForProductionProcessByMachineId = s.PlanningForProductionProcessByMachineId;
                        Group1.Add(schedulerDisplay);
                    }
            ...
            if (Lineres?.Any() == true)
            {
                Lineres.ForEach(s =>
                {
                    var exits = Group1.Where(w => w.Id == s.FixAssetMachineNameRequipmentId).FirstOrDefault();
                    if (exits == null)
                    {
                        SchedulerDisplay schedulerDisplay = new SchedulerDisplay();
                        schedulerDisplay.Id = s.FixAssetMachineNameRequipmentId;
                        schedulerDisplay.Text = s.PlanningForProductionProcessByMachine;
                        schedulerDisplay.Color = GetRandomColor();
                        schedulerDisplay.PlanningForProductionProcessByMachineId = s.PlanningForProductionProcessByMachineId;
                        Group2.Add(schedulerDisplay);
                    }

                });
            }
            ViewBag.GroupData1 = Group1;
            ViewBag.GroupData1 = Group2;
            ViewBag.Group1 = Group1
                .Select(p => new
                {
                    id = p.Id,
                    text = p.Text,
                    color = p.Color  // stable dynamic color
                });
            ViewBag.Group2 = Group2
                 .Select(p => new
                 {
                     ...
                     text = p.Text,
                     color = p.Color, // stable dynamic color
                     parentId = p.Id == 149 ? 159 : 160
                 });
            List<ProductionPlanningScheduler> productionPlanningSchedulers = new List<ProductionPlanningScheduler>();
            long i = 1;
            if (Group1?.Any() == true)
            {
                Group1.ForEach(g =>
                {
                    ProductionPlanningScheduler productionPlanningScheduler = new ProductionPlanningScheduler();
                    productionPlanningScheduler.ParentId = 0;
                    productionPlanningScheduler.Id = i++;
                    productionPlanningScheduler.Text = g.Text;
                    productionPlanningScheduler.Type = "Main";
                    productionPlanningScheduler.ProductionPlanningSchedulerId = g.Id.Value;
                    productionPlanningScheduler.PlanningForProductionProcessByMachine=g.PlanningForProductionProcessByMachineId;
                    productionPlanningSchedulers.Add(productionPlanningScheduler);
                });
            }
            if (Group2?.Any() == true)
            {
                ...
                {
                    ProductionPlanningScheduler productionPlanningScheduler = new ProductionPlanningScheduler();
                    productionPlanningScheduler.ParentId = productionPlanningSchedulers.FirstOrDefault(f => f.Type == "Main" && f.PlanningForProductionProcessByMachine==g.PlanningForProductionProcessByMachineId)?.Id;
                    productionPlanningScheduler.Id = i++;
                    productionPlanningScheduler.Text = g.Text;
            ...
            var result = _context.ProductionPlanningScheduler.ToList();
            var query1 = from oal in _context.ProductionPlanningScheduler
            ...
            if (res1?.Any() == true)
            {
                ...
                {
                    r.Id = i++;
                    r.ParentId = productionPlanningSchedulers.FirstOrDefault(f => f.Type == "Parent" && f.ProductionPlanningSchedulerId == r.PlanningForProductionProcessByMachine)?.Id;
                    r.Progress = 50;
                    productionPlanningSchedulers.Add(r);
                });
            }

            return View(productionPlanningSchedulers);
        }
    }
