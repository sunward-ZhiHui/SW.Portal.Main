...
namespace Infrastructure.Repository.Query
{
    public class SimulationQueryRepository : QueryRepository<INPCalendarPivotModel>, ISimulationQueryRepository
    {
        ...
        private async Task<List<INPCalendarPivotModel>> SimulationMidMonth(DateRangeModel endDate, List<NAVRecipesModel> recipeList, List<NAVRecipesModel> _orderRecipeList)
        {
            ...
            using (var connection = CreateConnection())
            {
                var query = "select * from NavpostedShipment WHERE CompanyId  in(" + string.Join(',', intercompanyIds) + ") AND CAST(StockBalanceMonth AS Date) <='" + lastDay + "'  AND (IsRecived is null Or IsRecived=0)\r\n;";
                ...
                query += "select t1.CustomerId as DistId,t1.ToDate as ACMonth,t3.No as ItemNo,t1.Quantity as ACQty,t2.ItemId as SWItemId,t3.Description as ItemDesc,t1.CustomerId from Acentry t1 INNER JOIN AcentryLines t2 ON t1.ACEntryId=t2.ACEntryId INNER JOIN NAVItems t3 ON t2.ItemId=t3.ItemId\r\n" +
                ...
                query += "select * from NavItemCitemList;\r\n";
                query += "select * from NavSaleCategory;\r\n";
                query += "select * from NavitemLinks where SgItemId is not null AND MyItemId is not null;\r\n";
                query += "select * from ProductionSimulation where IsBmrticket=0 AND CAST(StartingDate AS Date)>='" + dateMonth1 + "'  AND CAST(StartingDate AS Date)<='" + datemonth12 + "' order by StartingDate desc;\r\n";

                query += "select * from ProductionSimulation where IsBmrticket=0 AND CAST(ProcessDate AS Date)>='" + dateMonth1 + "'  AND CAST(ProcessDate AS Date)<='" + datemonth12 + "' order by ProcessDate desc;\r\n";

                query += "SELECT t1.*,t2.PackQty from NavitemStockBalance t1 INNER JOIN navitems t2 ON t1.ItemId=t2.ItemId Where t1.StockBalWeek=1 AND t2.ItemId is Not NUll and MONTH(t1.StockBalMonth) =" + month + " AND YEAR(t1.StockBalMonth)=" + year + ";\r\n";

                query += "SELECT t1.*,t2.PackQty from NavitemStockBalance t1 INNER JOIN navitems t2 ON t1.ItemId=t2.ItemId Where (t1.StockBalWeek is null OR t1.StockBalWeek=" + weekofmonth + ") AND t2.ItemId is Not NUll and MONTH(t1.StockBalMonth) =" + month + " AND YEAR(t1.StockBalMonth)=" + year + ";\r\n";
                query += "select t1.* from SimulationAddhoc t1 where  CAST(t1.ShipmentDate AS Date)>='" + dateMonth1 + "'  AND CAST(t1.ShipmentDate AS Date)<='" + datemonth12 + "';\r\n";

                query += "select * from GroupPlaningTicket where CompanyId="+ endDate.CompanyId + " AND CAST(StartDate AS Date)>='" + dateMonth1 + "'  AND CAST(StartDate AS Date)<='" + datemonth12 + "' order by StartDate desc;\r\n";
                query += "select s.No,s.ItemId,s.Description,s.PackQty,s.CompanyId from NAVItems s;";
                query += "select t1.*,t3.MethodCodeID,t4.MethodName,t2.Description2 as GenericCodeDescription2 from NAVItems t1\r\nINNER JOIN GenericCodes t2 ON t1.GenericCodeId=t2.GenericCodeId\r\nINNER JOIN NavMethodCodeLines t3 ON t1.ItemId=t3.ItemID\r\nINNER JOIN NavMethodCode t4 ON t4.MethodCodeId=t3.MethodCodeId\r\nINNER JOIN NavItemCitemList t5 ON t5.NavItemId=t1.ItemID\r\nINNER JOIN Acitems t6 ON t5.NavItemCustomerItemId=t6.DistACID\r\n" +
            ...
            itemMasterforReport.ForEach(ac =>
            {
                ...
                if (!genericId.Exists(g => g == ac.GenericCodeId) && ac.GenericCodeId != null)
                {
                    ...
                    var custmerItem = acEntriesList.FirstOrDefault(f => f.NavItemId == ac.ItemId);
                    custmerItem = custmerItem != null ? custmerItem : new Acitems();
                    if (custmerItem.DistAcid > 0)
                    {
                        ...
                        var prodRecipe = "";
                        var currentBatch = "";
                        var methodeCodeID = ac.MethodCodeId;
                        ...
                        MethodCodeList.Add(new INPCalendarPivotModel
                        {
                            ...
                            MethodCodeId = ac.MethodCodeId > 0 ? ac.MethodCodeId : -1,
                            ...
                            MethodCode = ac.MethodCodeId > 0 ? ac.MethodName : "No MethodCode",
