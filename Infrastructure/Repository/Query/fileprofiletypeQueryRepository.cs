    ...
    public class FileprofiletypeQueryRepository : QueryRepository<Fileprofiletype>, IFileprofileQueryRepository
    {
        ...
        public async Task<IReadOnlyList<DocumentsModel>> GetAllFileProfileDocumentAsync()
        {
            try
            {
                ...
                var query = "select     Name,   CASE WHEN  IsExpiryDate IS NULL THEN 0 ELSE  IsExpiryDate END AS IsExpiryDate,   CASE WHEN  IsDuplicateUpload IS NULL THEN 0 ELSE  IsDuplicateUpload END AS IsDuplicateUpload,    IsExpiryDate as IsExpiryDates,    ProfileID,    FileProfileTypeID,    AddedByUserID,    ModifiedByUserID,     ParentID,    SessionID,    IsDelete,    DeleteByUserID,    DeleteByDate,    DynamicFormID,   ROW_NUMBER() OVER(ORDER BY name) AS UniqueNo,Name as Filename,    FileProfileTypeID as DocumentID,    CASE WHEN ModifiedByUserID >0 THEN ModifiedDate ELSE AddedDate END AS AddedDate   from FileProfileType where (IsDelete = 0 or IsDelete is null)";
